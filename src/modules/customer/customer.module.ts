...
export function loadCustomerModule(container: Container): void {
  container
    .bind<ICustomerReadRepository>(CUSTOMER_TYPES.CustomerReadRepository)
    .to(CustomerReadRepositoryImpl)
    .inSingletonScope();
  container
    .bind<ICustomerWriteRepository>(CUSTOMER_TYPES.CustomerWriteRepository)
    .to(CustomerWriteRepositoryImpl)
    .inSingletonScope();

  container
    .bind<CreateCustomerCommand>(CUSTOMER_TYPES.CreateCustomerCommand)
    .to(CreateCustomerCommand);

  container
    .bind<CustomerCreatedEventHandler>(CUSTOMER_TYPES.CustomerCreatedEventHandler)
    .to(CustomerCreatedEventHandler);

  container
    .bind<CustomerMapper>(CUSTOMER_TYPES.CustomerMapper)
    .to(CustomerMapper)
    .inSingletonScope();
